...
public class Level_09_SwitchPageObject extends BaseTest {
	...
	public void TC4_Switch_Page_Object() {
		...
		orderPageObject = addressPageObject.clickToLinkOrder();
		Assert.assertEquals(orderPageObject.getTextTitle(), "My account - Orders");
		downloadProductPageObject= orderPageObject.clickToLinkDownloadProduct();
		Assert.assertEquals(downloadProductPageObject.getTextTitle(), "My account - Downloadable products");
		backInStockSubscriptionsPagetObject = downloadProductPageObject.clickToBackInStockSubscription();
		Assert.assertEquals(backInStockSubscriptionsPagetObject.getTextTitle(), "My account - Back in stock subscriptions");
		rewardPointPageObject = backInStockSubscriptionsPagetObject.clickToLinkRewardPoint();
		Assert.assertEquals(rewardPointPageObject.getTextTitle(), "My account - Reward points");
		changePasswordPageObject = rewardPointPageObject.clickToLinkChangePassword();
		Assert.assertEquals(changePasswordPageObject.getTextTitle(), "My account - Change password");
		myProductReviewPageObject = changePasswordPageObject.clickToLinkMyProductReview();
